...
#if !defined(BUILD_ESP32) && !defined(BUILD_NSPIRE)
...
#ifdef BUILD_NSPIRE
	(void)s;
	return (uint32_t)(((uint64_t)get_uticks() * CMOS_FREQ) / 1000000);
#else
	struct timespec ts;
...
void cmos_update_irq(CMOS *s)
{
	...
	if (s->data[RTC_REG_B] & REG_B_PIE) {
		...
		if ((int32_t)d >= 0) {
			...
			/* Do not replay missed host-time periods on a slow emulator. */
			s->irq_timeout = (now + s->irq_period) &
				~(s->irq_period - 1);
		}
	}
}
